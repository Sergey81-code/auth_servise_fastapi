...
async def check_user_permissions(target_user: User, current_user: User) -> bool:
    ...
    if (
        ...
        and PortalRole.ROLE_PORTAL_ADMIN not in target_user.roles
    ):
        return True
...
async def fetch_user_or_raise(
    user_id: UUID, current_user_roles: list, session: AsyncSession
) -> User:
    target_user = await get_user_by_id_action(user_id, session)
    if target_user is None:
        if (
            PortalRole.ROLE_PORTAL_ADMIN in current_user_roles
            or PortalRole.ROLE_PORTAL_SUPERADMIN in current_user_roles
        ):
            raise HTTPException(
                status_code=404, detail=f"User with id {user_id} not found."
            )
