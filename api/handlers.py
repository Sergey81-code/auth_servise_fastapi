...
from api.models import ActivateUserResponse, DeleteUserResponse, UpdateUserRequest, UpdatedUserResponse, UserCreate, ShowUser
...
async def _get_user_by_id(user_id: UUID, db: AsyncSession) -> ShowUser | None:
    async with db as session:
        async with session.begin():
            user_dal = UserDAL(session)
            received_user_id = await user_dal.get_user_by_id(
                user_id
            )
...
async def update_user_by_id(
    ...
    if updated_user_params == {}:
        raise HTTPException(status_code=422, detail="At least one parameter for user update info should be proveded")
    user = await _get_user_by_id(user_id, db)
    if user is None:
        raise HTTPException(status_code=404, detail=f'User with id {user_id} not found.')
