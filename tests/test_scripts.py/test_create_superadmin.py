...
from scripts.create_superadmin import create_superadmin, prompt_for_superadmin_credentials
from utils.hashing import Hasher
from utils.roles import PortalRole

@patch("builtins.input", side_effect=["test@example.com", "StrongPass1!", "Super", "Admin"])
async def test_prompt_for_superadmin_credentials(mock_input):
    with patch("scripts.create_superadmin.create_superadmin", new_callable=AsyncMock) as mock_create_superadmin:
        await prompt_for_superadmin_credentials()
        mock_create_superadmin.assert_called_once_with("test@example.com", "StrongPass1!", "Super", "Admin", ANY)


async def test_prompt_for_superadmin_credentials_invalid_email():
    with patch("builtins.input", side_effect=["invalid_email", "test@example.com", "StrongPass1!", "Super", "Admin"]),\
        patch("scripts.create_superadmin.create_superadmin", new_callable=AsyncMock) as mock_create_superadmin:
        await prompt_for_superadmin_credentials()
        mock_create_superadmin.assert_called_once_with("test@example.com", "StrongPass1!", "Super", "Admin", ANY)


async def test_prompt_for_superadmin_credentials_invalid_password():
    with patch("builtins.input", side_effect=["test@example.com", "invalid-password", "StrongPass1!", "Super", "Admin"]),\
        patch("scripts.create_superadmin.create_superadmin", new_callable=AsyncMock) as mock_create_superadmin:
        await prompt_for_superadmin_credentials()
        mock_create_superadmin.assert_called_once_with("test@example.com", "StrongPass1!", "Super", "Admin", ANY)
...
async def test_create_superadmin_already_exists(async_session_test, create_user_in_database):
    first_user_data = {
        "user_id": uuid4(),
        "email": "test@example.com",
        "password": "StrongPass1!",
