...
async def test_add_admin_role_to_user_by_superadmin(
    ...
    resp = client.patch(
        ...
        headers=await create_test_auth_headers_for_user(user_data_who_promoted["email"]),
    )
...
async def test_add_admin_role_to_user_by_superadmin_invalid_id_error(
    ...
    assert resp.json() == {"detail":[{"type":"uuid_parsing","loc":["query","user_id"],"msg":"Input should be a valid UUID, invalid length: expected length 32 for simple format, found 3","input":f"{invalid_user_id}","ctx":{"error":"invalid length: expected length 32 for simple format, found 3"}}]}
...
@pytest.mark.parametrize("promoted_user_data, promoter_data, expected_status_code, expected_detail",[
    (
        {
            "user_id": uuid4(),
...
async def test_invalid_admin_privilege_assignment(
    ...
    resp = client.patch(
        ...
        headers=await create_test_auth_headers_for_user(user_who_promoted_data["email"]),
    )
